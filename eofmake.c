...
void initialize(char * key_file, char * state_file, char* sbox_file, char* inv_sbox_file,char* Init_Chipper_file) {
    initFromFile(0,KEY_SIZE, key, key_file);
    initFromFile(0,STATE_SIZE, state, state_file);
    initFromFile(0,SBOX_SIZE, sbox, sbox_file);
    initFromFile(0,SBOX_SIZE, inv_sbox, inv_sbox_file);
    initFromFile(0,KEY_SIZE,  initChipper, Init_Chipper_file);
...
/**********************************************************************************************/
void initFromFile(int blockSize,int size, uint8_t matrix[size][size], char * file_name) {
    FILE *file;
    ...
    if (size == STATE_SIZE) { 
        for(int c=0; c<size; c++) {
            for(int r=0; r<size; r++) {
                if(fscanf(file, "%x", &input_var)!=EOF)
